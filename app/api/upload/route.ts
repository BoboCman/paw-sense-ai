...
// Increase the body size limit (this is the correct way for App Router)
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Create a proper Uint8Array from the ArrayBuffer
    ...
    try {
      ...
      const response = await fetch(webhookUrl.toString(), {
        method: "POST",
        headers: {
          "Content-Type": videoFile.type || "video/mp4", // Set Content-Type to match the video file type
          // Remove any additional headers that might interfere
        },
        body: uint8Array, // Send the raw binary data as Uint8Array
